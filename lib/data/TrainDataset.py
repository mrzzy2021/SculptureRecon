...
def load_trimesh(root_dir):
    ...
    for i, f in enumerate(folders):
        sub_name = f
        meshs[sub_name] = trimesh.load(os.path.join(root_dir, f, sub_name + '.obj'))
...
class TrainDataset(Dataset):
    ...
    def __init__(self, opt, phase='train', is_sculpture = False):
        self.opt = opt
        self.projection_mode = 'orthogonal'
        self.is_sculpture = is_sculpture

        # Path setup
        if self.is_sculpture:
            self.root = self.opt.sculptureroot#../Sculptures
        else:
            self.root = self.opt.dataroot#../RenderPeople
    ...
    def get_subjects(self):
        ...
        if self.is_train:
            ...
            return sorted(list(set(var_subjects)))
