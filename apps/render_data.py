...
def render_prt_ortho(out_path, folder_name, subject_name, shs, rndr, rndr_uv, im_size, angl_step=4, n_light=1, pitch=[0]):
    ...
    text_file = os.path.join(folder_name, 'material_0.png')
    if not os.path.exists(text_file):
        print('ERROR: dif file does not exist!!', text_file)
        return
    mtl_file = os.path.join(folder_name, 'material_0.mtl')
    ...
    # copy obj file
    cmd_obj = 'cp %s %s' % (mesh_file, os.path.join(out_path, 'GEO', 'OBJ', subject_name))
    print(cmd_obj)
    os.system(cmd_obj)
    
    cmd_png = 'cp %s %s' % (text_file, os.path.join(out_path, 'GEO', 'OBJ', subject_name))
    print(cmd_png)
    os.system(cmd_png)
    
    cmd_mtl = 'cp %s %s' % (mtl_file, os.path.join(out_path, 'GEO', 'OBJ', subject_name))
    print(cmd_mtl)
    os.system(cmd_mtl)
...
if __name__ == '__main__':
    ...
    for subject in os.listdir(args.input):
        subject_name = subject
        sub_path = args.input + '/' + subject
        render_prt_ortho(args.out_dir, sub_path, subject_name, shs, rndr, rndr_uv, args.size, 1, 1, pitch=[0])